class TriTas:
    ...
    def trier(tableau):
        ...
        # les deux prochaines lignes sont l'équivalent de la fonction creation_tas_max()
        for i in range(len(tableau) // 2 - 1, -1, -1):
            TriTas.tamisage_max(tableau, len(tableau), i)
        print(f'tableau en tas:\t{tableau}')

        # On classe les éléments en permutant la racine (le plus grand elements) à la dernière position non triée
